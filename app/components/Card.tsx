...
type Props = {
  card: { title: string; price: string; url: string; isBestSeller: boolean }
  className?: string
}

export default function CardDemo({
  card: { title, price, url, isBestSeller },
  className,
}: Props) {
  return (
    <Card
    ...
    >
      <div className="relative">
        <div className="aspect aspect-h-3 aspect-w-3 sm:aspect-h-4">
          <img className="object-cover object-center" alt="avatar" src={url} />
        </div>
        {isBestSeller && (
      ...
      <div className="mt-auto flex flex-1 flex-col p-[18px]">
        ...
        <div className="mt-auto flex flex-col items-center gap-3 lg:flex-row lg:gap-[6px]">
          <Link className="w-full flex-1" href="/shop/random">
            <Button
