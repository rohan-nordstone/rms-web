...
export default function DropDown({ className }: Props) {
  const DropdownContent = ({
    ...
    <Dropdown placement="bottom-end" className="relative">
      <DropdownTrigger>
        <summary
          className={cn(
            "btn border-none bg-transparent hover:bg-transparent",
            className ?? className
          )}
        >
          <svg
  ...
  return (
    <div className="flex flex-wrap gap-4">
      <DropdownContent key="shadow" color="default" variant="shadow" />
    </div>
  )
}
