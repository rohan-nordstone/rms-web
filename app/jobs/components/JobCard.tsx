...
export default async function JobCard({ job }: Props) {
  const session = await getServerSession(authOptions)

  let isJobSaved: boolean = false
  ...
  try {
    ...
    const { data } = await axios.get<boolean>(
      `${process.env.NEXT_PUBLIC_BACKEND_URL}/jobs/${job.id}/isJobSaved`,
      {
        headers: {
          Authorization,
        },
  ...
  return (
    <Card className="relative flex flex-1 flex-col p-4 sm:p-6 md:p-4 lg:p-6">
      <div className="flex items-center gap-7 pb-8 pt-2">
        <Image
        ...
        <div className="flex flex-1 flex-col justify-center">
          <Link
            href={`/jobs/${job.id}`}
            className="w-[90%] text-2xl font-bold leading-[30.50px] text-black hover:underline"
          >
            {job.position}
          </Link>
          <Link
            href={"/profile/visitor_newsroom"}
            className="text-base font-light leading-relaxed text-secondary"
          >
            {job.station.name}
          </Link>
        </div>
      ...
      <div className="flex flex-wrap gap-3 rounded-lg bg-transparent xl:justify-center xl:bg-zinc-100">
      ...
      <ul className="my-6 list-disc space-y-2 pl-5">
        <li className="line-clamp-[10] text-base font-light leading-relaxed text-black">
          {job.jobDescription}
        </li>
      </ul>
      <div className="mt-auto flex items-center justify-between">
        <Badge
          className={cn(
            ...
            job.isHiring ? "bg-primary" : "bg-secondary"
          )}
        >
          <svg
            width="16"
        ...
        <div className="flex items-center gap-2">
          ...
          <p className="text-sm font-light text-zinc-500">
            {convertTimestamp(new Date(job.updatedAt))}
          </p>
        </div>
      </div>
    </Card>
